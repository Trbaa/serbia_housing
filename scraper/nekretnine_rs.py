...
def scrape_all_pages(listing_page,context,start_url,cursor,conn,max_pages = None):
    ...
    while True:
        ...
        for i,url in enumerate(urls,start = 1):
            ...
            seen_urls.add(url)
            try:
                status = check_url_status(cursor,url,"nekretnine_rs")
                if status == URL_COMPLETE:
                    skipped_count += 1
                    print(f"  [{i}/{len(urls)}] [NEKRETNINE] Preskacem (kompletan): {url}")
                    continue
                ...
                if item is None:
                    continue
                oglas_id_backup = extract_oglas_id(url, "nekretnine_rs") 
                ...
                if status == URL_NEW:
                    ...
                    try:
                        rows = update_full_row_nekretnine(cursor,item)
                    except Exception:
                        conn.rollback()
                        conn,cursor = ensure_connection(conn,cursor,get_scraping_db_connection_params)
                        rows = update_full_row_nekretnine(cursor,item)
                    ...
                    updated_count += 1
                    print(f"  [{i}/{len(urls)}] [NEKRETNINE] UPDATE ({rows} red): {url}")
                total = inserted_count + updated_count
...
def run_nekretnine(max_pages = None):
    conn = None
    cursor = None
    ...
    try:
        ...
        with sync_playwright() as p:
            ...
            start_url = "https://www.nekretnine.rs/stambeni-objekti/stanovi/izdavanje-prodaja/prodaja/grad/beograd/lista/po-stranici/20/"
